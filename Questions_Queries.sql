...
-- 13. What is Turnover_Rate By Age_Group?
SELECT
a.Age_Group, a.Total_Count, t.Terminated_Count,
t.Terminated_Count * 1.0/ a.Total_Count AS Turnover_Rate
FROM(
SELECT 	
        CASE 
		 WHEN Age <= 30 THEN '18-30'
		 ...
		 ELSE '46-60'
	    END AS Age_Group
	, COUNT(*) AS Total_Count FROM [Human Resources]
GROUP BY 
        CASE 
		 WHEN Age <= 30 THEN '18-30'
		 WHEN Age BETWEEN 31 AND 45 THEN '31-45'
...
SELECT 	
        CASE 
		 ...
		 ELSE '46-60'
	    END AS Age_Group
	, COUNT(*) AS Terminated_Count FROM [Human Resources]
WHERE termdate <='2020-12-31'
GROUP BY 
        CASE 
		 WHEN Age <= 30 THEN '18-30'
		 WHEN Age BETWEEN 31 AND 45 THEN '31-45'
		 ELSE '46-60'
	    END ) AS t
ON a.Age_Group = t.Age_Group


-- 14. What is Turnover_Rate By Gender?
SELECT a.gender,
a.Total_Count, t.Terminated_Count,
t.Terminated_Count * 1.0/ a.Total_Count AS Turnover_Rate
FROM(
SELECT gender,COUNT(*) AS Total_Count FROM [Human Resources]
GROUP BY gender) AS a
INNER JOIN (
SELECT gender, COUNT(*) AS Terminated_Count FROM [Human Resources]
WHERE termdate <='2020-12-31'
GROUP BY gender) AS t
ON a.gender= t.gender
